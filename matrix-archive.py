...
async def create_client() -> AsyncClient:
    ...
    logging.info(await client.login(password, DEVICE_NAME))
    client.load_store()
    ...
    logging.info("Importing keys. This may take a while...")
    logging.info(await client.import_keys(room_keys_path, room_keys_password))
...
async def main() -> None:
    try:
        ...
        logging.info("Synced te client with the server")
        for room_id, room in client.rooms.items():
...
if __name__ == "__main__":
    ...
    logging.info("parsed args and appendend rooms")
